...
int main() {
    ...
    printf("\nExpanded Code:\n");
    for (int k = 0; k < j; k++) {
        if (strncmp(code[k], "end", 3) == 0) {
            break;
        }
        else if (strncmp(code[k], ".code", 5) == 0) {
        // Skip the .code line
        continue;
        } 
        else {
            // Expand Macro Call in Code
            int foundMacro = 0;
            sscanf(code[k], "%s", macroProcesses[0]); // Extract Macro Name
            for (int l = 1; l < mntIndex; l++) {
                if (strcmp(macroProcesses[0], mnt[l].name) == 0) {
                    // Found Macro in MNT
                    foundMacro = 1;
                    for (int m = mnt[l].mdtIndex; strncmp(mdt[m].card, "mend", 4) != 0; m++) {
                        printf("%s", mdt[m].card);
                    }
                    break;
                }
            ...
            if (!foundMacro) {
                // If not a macro name, print the line as it is
                printf("%s", code[k]);
            }
        }
    }
